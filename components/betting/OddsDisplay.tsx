  ...
  return (
    <div className="flex gap-2">
      {odds.map((odd) => (
        <div
          key={odd.label}
          className="flex-1 flex flex-col items-center gap-1 px-3 py-2 bg-bg-tertiary border border-border rounded-lg opacity-60 cursor-not-allowed select-none"
        >
          <span className="font-mono text-[10px] text-muted">{odd.label}</span>
          <span className="font-mono text-xs text-[#e8e8f0]">🔒 {odd.value}</span>
        </div>
      ))}
    </div>
  );
